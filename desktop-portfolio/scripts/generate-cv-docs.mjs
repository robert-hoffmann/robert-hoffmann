...
function renderFooterLinks(links) {
  return links.map(link => renderLink(link.href, link.label)).join(' - ')
}
    ...
    * {
    ...
    .mark {
      ...
      background:
        ...
        linear-gradient(135deg, #16324c, #246a8d);
  ...
  <article class="sheet" aria-label="${escapeAttribute(doc.name)} CV">
    ...
    <header class="hero">
      <div class="hero-grid">
        <div>
          <div class="identity">
            <div class="mark">${escapeHtml(doc.mark)}</div>
            <div>
      ...
      <aside class="rail">
        ...
        <section>
          ...
          <div class="chips">
            ...
            ${doc.emphasisStack.map(label => renderChip(label, 'dark')).join('\n            ')}
          </div>
        </section>
    ...
    <footer class="footer">
      <span>${renderFooterLinks(doc.footer.links)}</span>
      <span>${escapeHtml(doc.footer.text)}</span>
    </footer>
  </article>
</body>
</html>
