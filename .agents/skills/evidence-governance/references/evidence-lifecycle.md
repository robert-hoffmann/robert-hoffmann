...
`- agent-ops/         # Cross-cutting agent governance records
```
...
### Decision-Memory Root

Canonical architecture decision records live in `docs/adr/`.

Required decision-memory files:

1. `docs/adr/index.md` - human-readable index of active ADRs.
2. `docs/adr/registry.json` - machine-readable ADR registry.
3. `docs/adr/claim-register.json` - claim confidence register for governance rationale.
...
- ADR and claim registries are canonical in `docs/adr/` and referenced from evidence records.
...
- ADR and claim records must include owners and review dates for lifecycle accountability.
...
- ADR registry and claim register entries past `reviewBy` are treated as stale policy inputs.
...
- Superseded artifacts should be clearly marked (for example moved to a `superseded/` subfolder within the phase folder).
- Do not remove superseded ADRs; mark them `deprecated` or `superseded` in the registry.
...
- High-risk and `L2`/`L3` runs include `decision` metadata with ADR references.
...
- ADR and claim gate outputs should be reproducible from deterministic scripts.
...
6. **Decision drift**: structural changes with no ADR alignment update.
7. **Claim drift**: stale or unverified claims used as blocking governance rationale.
