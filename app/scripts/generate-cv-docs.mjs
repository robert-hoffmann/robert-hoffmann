#!/usr/bin/env node
/**
 * Regenerate localized CV PDFs and design-side review artifacts from curated app data.
 *
 * Source data:
 *   src/data/docs/cv.ts
 *   design/identity/profile_400x400.jpg
 *   design/identity/x-banner-1500x500.jpeg
 *
 * Outputs:
...
const IDENTITY_DIR    = join(PROJECT_ROOT, 'design/identity')
const CV_REVIEW_DIR   = IDENTITY_DIR
const PUBLIC_DOCS_DIR = join(PROJECT_ROOT, 'public/docs')
const HERO_FILENAME   = 'robert-hoffmann-cv-hero.jpeg'
const HERO_PATH       = join(CV_REVIEW_DIR, HERO_FILENAME)
const CV_LOCALES      = ['en', 'fr']
...
function cvArtifactSuffix(locale) {
  return locale === 'en' ? '' : `-${locale}`
}

function cvHtmlPath(locale) {
  return join(CV_REVIEW_DIR, `robert-hoffmann-cv${cvArtifactSuffix(locale)}.html`)
}

function cvPdfPath(locale) {
  return join(PUBLIC_DOCS_DIR, `robert-hoffmann-cv${cvArtifactSuffix(locale)}.pdf`)
}

function cvPngPath(locale) {
  return join(CV_REVIEW_DIR, `robert-hoffmann-cv${cvArtifactSuffix(locale)}.png`)
}
// #endregion Helpers
...
function renderHtml(doc) {
  ...
  return `<!doctype html>
<!-- Generated by scripts/generate-cv-docs.mjs. Do not edit directly; see docs/workflows/cv-workflow.md. -->
<html lang="${escapeAttribute(doc.locale)}">
<head>
  <meta charset="utf-8">
...
<body>
  <article class="sheet" aria-label="${escapeAttribute(`${doc.name} ${doc.labels.documentKind}`)}">
    <div class="topbar">
      <div><strong>${escapeHtml(doc.name)}</strong></div>
      <div class="center">${escapeHtml(doc.labels.documentKind)}</div>
    ...
    <header class="hero">
      <div class="hero-grid">
        <div>
          <div class="identity">
            <div class="mark" role="img" aria-label="${escapeAttribute(`${doc.name} ${doc.labels.profilePhoto}`)}"></div>
            <div>
    ...
    <main class="main">
      <aside class="rail">
        ...
        <section>
          <h2>${escapeHtml(doc.labels.deliveryReliability)}</h2>
          <div class="chips">
        ...
        <section>
          <h2>${escapeHtml(doc.labels.languages)}</h2>
          <div class="chips">
        ...
        <section>
          <h2>${escapeHtml(doc.labels.certifications)}</h2>
          <div class="chips">
        ...
        <section>
          <h2>${escapeHtml(doc.labels.portfolioProof)}</h2>
        ...
        <section>
          <h2>${escapeHtml(doc.labels.operatingMode)}</h2>
          <ul>
      ...
      <div class="content">
        <section>
          <h2>${escapeHtml(doc.labels.currentValue)}</h2>
          <div class="callout">
        ...
        <section>
          <h2>${escapeHtml(doc.labels.selectedExperience)}</h2>
          <div class="timeline">
        ...
        <section class="impact-projects-section">
          <h2>${escapeHtml(doc.labels.selectedImpactProjects)}</h2>
...
async function writeHtml(doc) {
  await mkdir(CV_REVIEW_DIR, { recursive : true })
  await writeFile(cvHtmlPath(doc.locale), normalizeGeneratedHtml(renderHtml(doc)), 'utf8')
}

async function renderPdfAndPreview(doc) {
  ...
  try {
    ...
    await page.goto(pathToFileURL(cvHtmlPath(doc.locale)).href, { waitUntil : 'load' })
    await page.locator('.sheet').waitFor({ state : 'visible' })
    await page.pdf({
      path              : cvPdfPath(doc.locale),
      format            : 'A4',
      printBackground   : true,
      preferCSSPageSize : true,
    })
    await page.emulateMedia({ media : 'screen' })
    await page.locator('.sheet').screenshot({ path : cvPngPath(doc.locale) })
  } finally {
    await browser.close()
  }
}
...
function identitySourcePath(fileName) {
  return join(IDENTITY_DIR, fileName)
}

async function validateIdentityAssets(doc) {
  const avatarSource = identitySourcePath(doc.identity.avatarSrc)
  const bannerSource = identitySourcePath(doc.identity.bannerSrc)

  await readNonEmptyArtifact(avatarSource, 'CV avatar source')
  await readNonEmptyArtifact(bannerSource, 'CV banner source')
}

async function generateFlattenedHeroBackground(doc) {
  await sharp(identitySourcePath(doc.identity.bannerSrc))
    .resize({
...
async function validateLocaleArtifacts(doc) {
  await readNonEmptyArtifact(HERO_PATH, 'flattened hero')
  const html = await readNonEmptyArtifact(cvHtmlPath(doc.locale), `${doc.locale} HTML`)
  const pdf  = await readNonEmptyArtifact(cvPdfPath(doc.locale), `${doc.locale} PDF`)
  await readNonEmptyArtifact(cvPngPath(doc.locale), `${doc.locale} PNG`)

  if (!html.includes('Do not edit directly')) {
    throw new Error(`Generated ${doc.locale} HTML is missing the do-not-edit notice`)
  }

  if (!html.includes(`lang="${doc.locale}"`)) {
    throw new Error(`Generated ${doc.locale} HTML is missing the expected lang attribute`)
  }

  if (pdf.subarray(0, 4).toString('utf8') !== '%PDF') {
    throw new Error(`Generated ${doc.locale} PDF does not begin with %PDF`)
  }

  const metadata = await sharp(cvPngPath(doc.locale)).metadata()

  if (metadata.width !== A4_PREVIEW.width || metadata.height !== A4_PREVIEW.height) {
    throw new Error(
      `Generated ${doc.locale} PNG is ${metadata.width}x${metadata.height}; expected ${A4_PREVIEW.width}x${A4_PREVIEW.height}`,
    )
  }
}

async function validateArtifacts(documents) {
  await Promise.all(documents.map(validateLocaleArtifacts))
}
// #endregion Artifact Generation

// #region CLI
async function main() {
  const documentsByLocale = await loadCvDocuments()
  const documents = CV_LOCALES.map(locale => {
    const doc = documentsByLocale[locale]

    if (!doc) {
      throw new Error(`Missing CV document for locale: ${locale}`)
    }

    return doc
  })

  await Promise.all(documents.map(validateIdentityAssets))
  await generateFlattenedHeroBackground(documents[0])

  for (const doc of documents) {
    await writeHtml(doc)
    await renderPdfAndPreview(doc)
  }

  await validateArtifacts(documents)

  console.log([
    'CV docs generated:',
    'Runtime:',
    ...documents.map(doc => `- ${artifactLabel(cvPdfPath(doc.locale))}`),
    'Review artifacts:',
    `- ${artifactLabel(HERO_PATH)}`,
    ...documents.flatMap(doc => [
      `- ${artifactLabel(cvHtmlPath(doc.locale))}`,
      `- ${artifactLabel(cvPngPath(doc.locale))} (${A4_PREVIEW.width}x${A4_PREVIEW.height})`,
    ]),
  ].join('\n'))
}
